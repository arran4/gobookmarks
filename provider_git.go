...
func userDir(user string) string {
	...
	return filepath.Join(Config.LocalGitPath, hex.EncodeToString(h[:]))
}
...
func passwordPath(user string) string {
	h := sha256.Sum256([]byte(user))
	return filepath.Join(Config.LocalGitPath, hex.EncodeToString(h[:]), ".password")
}
...
func (GitProvider) CreateUser(ctx context.Context, user, password string) error {
	gp := GitProvider{}
	if err := gp.CreateRepo(ctx, user, nil, Config.GetRepoName()); err != nil {
		return err
	}
	p := passwordPath(user)
