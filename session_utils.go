...
func (c *Configuration) sanitizeSession(w http.ResponseWriter, r *http.Request, session *sessions.Session, err error) (*sessions.Session, error) {
	if err == nil {
		return session, nil
	}
	...
	if (errors.As(err, scErr) && scErr.IsDecode() && !scErr.IsInternal() && !scErr.IsUsage()) || errors.Is(err, securecookie.ErrMacInvalid) {
		...
		session, _ = c.SessionStore.New(r, c.SessionName)
		return session, nil
	}
	return session, err
}
