...
func TestGitProviderCreateAndGet(t *testing.T) {
	tmp := t.TempDir()
	cfg := NewConfiguration(Config{LocalGitPath: tmp})
	ctx := context.WithValue(context.Background(), ContextValues("configuration"), cfg)
	p := GitProvider{}
	user := "alice"
	text := "Category: Test\nhttp://example.com test"
	if err := p.CreateRepo(ctx, user, nil, cfg.GetRepoName()); err != nil {
		t.Fatalf("CreateRepo: %v", err)
	}
	gi, err := os.ReadFile(filepath.Join(userDir(user, tmp), ".gitignore"))
	if err != nil {
...
func TestGitRepoExists(t *testing.T) {
	...
	p := GitProvider{}
	user := "carol"
	exists, err := p.RepoExists(ctx, user, nil, cfg.GetRepoName())
	if err != nil {
...
func TestGitPasswordLifecycle(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(userDir(user, tmp), ".git")); err != nil {
