...
func NewServeCmd(version, commit, date string) *ServeCommand {
	return &ServeCommand{
...
func (c *ServeCommand) Run(args []string) error {
	...
	c.FlagSet.StringVar(&cfgFlag, "config", "", "path to config file")
	c.FlagSet.StringVar(&ghIDFlag, "github-client-id", "", "GitHub OAuth client ID")
	c.FlagSet.StringVar(&ghSecretFlag, "github-secret", "", "GitHub OAuth client secret")
	c.FlagSet.StringVar(&glIDFlag, "gitlab-client-id", "", "GitLab OAuth client ID")
	c.FlagSet.StringVar(&glSecretFlag, "gitlab-secret", "", "GitLab OAuth client secret")
	c.FlagSet.StringVar(&urlFlag, "external-url", "", "external URL")
	c.FlagSet.StringVar(&nsFlag, "namespace", "", "repository namespace")
	c.FlagSet.StringVar(&titleFlag, "title", "", "site title")
	c.FlagSet.StringVar(&faviconDirFlag, "favicon-cache-dir", "", "directory for cached favicons")
	c.FlagSet.StringVar(&faviconSizeFlag, "favicon-cache-size", "", "max size of favicon cache in bytes")
	c.FlagSet.StringVar(&commitsPerPageFlag, "commits-per-page", "", "commits per page")
	c.FlagSet.StringVar(&ghServerFlag, "github-server", "", "GitHub base URL")
	c.FlagSet.StringVar(&glServerFlag, "gitlab-server", "", "GitLab base URL")
	c.FlagSet.StringVar(&localGitPathFlag, "local-git-path", "", "directory for local git provider")
	c.FlagSet.StringVar(&dbProviderFlag, "db-provider", "", "SQL driver name")
	c.FlagSet.StringVar(&dbConnFlag, "db-conn", "", "SQL connection string")
	c.FlagSet.StringVar(&sessionKeyFlag, "session-key", "", "session cookie key")
	c.FlagSet.StringVar(&providerOrderFlag, "provider-order", "", "comma-separated provider order")
	c.FlagSet.BoolVar(&columnFlag, "css-columns", false, "use CSS columns")
	c.FlagSet.BoolVar(&noFooterFlag, "no-footer", false, "disable footer on pages")
	c.FlagSet.BoolVar(&devModeFlag, "dev-mode", false, "enable dev mode helpers")
	c.FlagSet.BoolVar(&dumpConfig, "dump-config", false, "print merged config and exit")
	...
	if c.FlagSet.Lookup("config").Value.String() != "" {
		configPath = cfgFlag
	}

	cfgSpecified := c.FlagSet.Lookup("config").Value.String() != "" || os.Getenv("GOBM_CONFIG_FILE") != ""
	...
	if c.FlagSet.Lookup("github-client-id").Value.String() != "" {
		cfg.GithubClientID = ghIDFlag
	}
	if c.FlagSet.Lookup("github-secret").Value.String() != "" {
		cfg.GithubSecret = ghSecretFlag
	}
	if c.FlagSet.Lookup("gitlab-client-id").Value.String() != "" {
		cfg.GitlabClientID = glIDFlag
	}
	if c.FlagSet.Lookup("gitlab-secret").Value.String() != "" {
		cfg.GitlabSecret = glSecretFlag
	}
	if c.FlagSet.Lookup("external-url").Value.String() != "" {
		cfg.ExternalURL = urlFlag
	}
	if c.FlagSet.Lookup("namespace").Value.String() != "" {
		cfg.Namespace = nsFlag
	}
	if c.FlagSet.Lookup("title").Value.String() != "" {
		cfg.Title = titleFlag
	}
	if c.FlagSet.Lookup("css-columns").Value.String() == "true" {
		cfg.CssColumns = columnFlag
	}
	if c.FlagSet.Lookup("no-footer").Value.String() == "true" {
		cfg.NoFooter = noFooterFlag
	}
	if c.FlagSet.Lookup("dev-mode").Value.String() == "true" {
		cfg.DevMode = BP(devModeFlag)
	}
	if c.FlagSet.Lookup("github-server").Value.String() != "" {
		cfg.GithubServer = ghServerFlag
	}
	if c.FlagSet.Lookup("favicon-cache-dir").Value.String() != "" {
		cfg.FaviconCacheDir = faviconDirFlag
	}
	if c.FlagSet.Lookup("favicon-cache-size").Value.String() != "" {
		if i, err := strconv.ParseInt(faviconSizeFlag, 10, 64); err == nil {
			cfg.FaviconCacheSize = i
		}
	}
	if c.FlagSet.Lookup("commits-per-page").Value.String() != "" {
		if i, err := strconv.Atoi(commitsPerPageFlag); err == nil {
			cfg.CommitsPerPage = i
		}
	}
	if c.FlagSet.Lookup("gitlab-server").Value.String() != "" {
		cfg.GitlabServer = glServerFlag
	}
	if c.FlagSet.Lookup("local-git-path").Value.String() != "" {
		cfg.LocalGitPath = localGitPathFlag
	}
	if c.FlagSet.Lookup("db-provider").Value.String() != "" {
		cfg.DBConnectionProvider = dbProviderFlag
	}
	if c.FlagSet.Lookup("db-conn").Value.String() != "" {
		cfg.DBConnectionString = dbConnFlag
	}
	if c.FlagSet.Lookup("session-key").Value.String() != "" {
		cfg.SessionKey = sessionKeyFlag
	}
	if c.FlagSet.Lookup("provider-order").Value.String() != "" {
	...
	log.Printf("gobookmarks: %s, commit %s, built at %s", c.Version, c.Commit, c.Date)
	SetVersion(c.Version, c.Commit, c.Date)
	RepoName = GetBookmarksRepoName()
