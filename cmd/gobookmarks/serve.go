...
func (c *ServeCommand) Short() string { return "Starts the gobookmarks server" }
func (c *ServeCommand) Long() string  { return `Starts the gobookmarks server, serving the web UI and API.` }
...
func (c *ServeCommand) Run(args []string) error {
	...
	fs := c.FlagSet()
	fs.StringVar(&cfgFlag, "config", "", "path to config file")
	fs.StringVar(&ghIDFlag, "github-client-id", "", "GitHub OAuth client ID")
	fs.StringVar(&ghSecretFlag, "github-secret", "", "GitHub OAuth client secret")
	fs.StringVar(&glIDFlag, "gitlab-client-id", "", "GitLab OAuth client ID")
	fs.StringVar(&glSecretFlag, "gitlab-secret", "", "GitLab OAuth client secret")
	fs.StringVar(&urlFlag, "external-url", "", "external URL")
	fs.StringVar(&nsFlag, "namespace", "", "repository namespace")
	fs.StringVar(&titleFlag, "title", "", "site title")
	fs.StringVar(&faviconDirFlag, "favicon-cache-dir", "", "directory for cached favicons")
	fs.StringVar(&faviconSizeFlag, "favicon-cache-size", "", "max size of favicon cache in bytes")
	fs.StringVar(&commitsPerPageFlag, "commits-per-page", "", "commits per page")
	fs.StringVar(&ghServerFlag, "github-server", "", "GitHub base URL")
	fs.StringVar(&glServerFlag, "gitlab-server", "", "GitLab base URL")
	fs.StringVar(&localGitPathFlag, "local-git-path", "", "directory for local git provider")
	fs.StringVar(&dbProviderFlag, "db-provider", "", "SQL driver name")
	fs.StringVar(&dbConnFlag, "db-conn", "", "SQL connection string")
	fs.StringVar(&sessionKeyFlag, "session-key", "", "session cookie key")
	fs.StringVar(&providerOrderFlag, "provider-order", "", "comma-separated provider order")
	fs.BoolVar(&columnFlag, "css-columns", false, "use CSS columns")
	fs.BoolVar(&noFooterFlag, "no-footer", false, "disable footer on pages")
	fs.BoolVar(&devModeFlag, "dev-mode", false, "enable dev mode helpers")
	fs.BoolVar(&dumpConfig, "dump-config", false, "print merged config and exit")
	...
	if fs.Lookup("config").Value.String() != "" {
		configPath = cfgFlag
	}

	cfgSpecified := fs.Lookup("config").Value.String() != "" || os.Getenv("GOBM_CONFIG_FILE") != ""
	...
	if fs.Lookup("github-client-id").Value.String() != "" {
		cfg.GithubClientID = ghIDFlag
	}
	if fs.Lookup("github-secret").Value.String() != "" {
		cfg.GithubSecret = ghSecretFlag
	}
	if fs.Lookup("gitlab-client-id").Value.String() != "" {
		cfg.GitlabClientID = glIDFlag
	}
	if fs.Lookup("gitlab-secret").Value.String() != "" {
		cfg.GitlabSecret = glSecretFlag
	}
	if fs.Lookup("external-url").Value.String() != "" {
		cfg.ExternalURL = urlFlag
	}
	if fs.Lookup("namespace").Value.String() != "" {
		cfg.Namespace = nsFlag
	}
	if fs.Lookup("title").Value.String() != "" {
		cfg.Title = titleFlag
	}
	if fs.Lookup("css-columns").Value.String() == "true" {
		cfg.CssColumns = columnFlag
	}
	if fs.Lookup("no-footer").Value.String() == "true" {
		cfg.NoFooter = noFooterFlag
	}
	if fs.Lookup("dev-mode").Value.String() == "true" {
		cfg.DevMode = BP(devModeFlag)
	}
	if fs.Lookup("github-server").Value.String() != "" {
		cfg.GithubServer = ghServerFlag
	}
	if fs.Lookup("favicon-cache-dir").Value.String() != "" {
		cfg.FaviconCacheDir = faviconDirFlag
	}
	if fs.Lookup("favicon-cache-size").Value.String() != "" {
		if i, err := strconv.ParseInt(faviconSizeFlag, 10, 64); err == nil {
			cfg.FaviconCacheSize = i
		}
	}
	if fs.Lookup("commits-per-page").Value.String() != "" {
		if i, err := strconv.Atoi(commitsPerPageFlag); err == nil {
			cfg.CommitsPerPage = i
		}
	}
	if fs.Lookup("gitlab-server").Value.String() != "" {
		cfg.GitlabServer = glServerFlag
	}
	if fs.Lookup("local-git-path").Value.String() != "" {
		cfg.LocalGitPath = localGitPathFlag
	}
	if fs.Lookup("db-provider").Value.String() != "" {
		cfg.DBConnectionProvider = dbProviderFlag
	}
	if fs.Lookup("db-conn").Value.String() != "" {
		cfg.DBConnectionString = dbConnFlag
	}
	if fs.Lookup("session-key").Value.String() != "" {
		cfg.SessionKey = sessionKeyFlag
	}
	if fs.Lookup("provider-order").Value.String() != "" {
