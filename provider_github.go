...
func (p GitHubProvider) GetTags(ctx context.Context, user string, token *oauth2.Token) ([]*Tag, error) {
	tags, _, err := p.client(ctx, token).Repositories.ListTags(ctx, user, AppConfig.GetRepoName(), &github.ListOptions{})
	if err != nil {
...
func (p GitHubProvider) GetBranches(ctx context.Context, user string, token *oauth2.Token) ([]*Branch, error) {
	bs, _, err := p.client(ctx, token).Repositories.ListBranches(ctx, user, AppConfig.GetRepoName(), &github.BranchListOptions{})
	if err != nil {
...
func (p GitHubProvider) GetCommits(ctx context.Context, user string, token *oauth2.Token, ref string, page, perPage int) ([]*Commit, error) {
	...
	cs, _, err := p.client(ctx, token).Repositories.ListCommits(ctx, user, AppConfig.GetRepoName(), opts)
	if err != nil {
...
func (p GitHubProvider) GetBookmarks(ctx context.Context, user, ref string, token *oauth2.Token) (string, string, error) {
	contents, _, resp, err := p.client(ctx, token).Repositories.GetContents(ctx, user, AppConfig.GetRepoName(), "bookmarks.txt", &github.RepositoryContentGetOptions{Ref: ref})
...
func (p GitHubProvider) getDefaultBranch(ctx context.Context, user string, client *github.Client, branch string) (string, error) {
	rep, resp, err := client.Repositories.Get(ctx, user, AppConfig.GetRepoName())
	if resp != nil && resp.StatusCode == 404 {
...
func (p GitHubProvider) createRef(ctx context.Context, user string, client *github.Client, sourceRef, branchRef string) error {
	gsref, resp, err := client.Git.GetRef(ctx, user, AppConfig.GetRepoName(), sourceRef)
	if resp != nil && resp.StatusCode == 404 {
		err = nil
	}
	if err != nil {
	...
	_, _, err = client.Git.CreateRef(ctx, user, AppConfig.GetRepoName(), &github.Reference{Ref: &branchRef, Object: gsref.Object})
	if err != nil {
...
func (p GitHubProvider) UpdateBookmarks(ctx context.Context, user string, token *oauth2.Token, sourceRef, branch, text, expectSHA string) error {
	...
	_, grefResp, err := client.Git.GetRef(ctx, user, AppConfig.GetRepoName(), branchRef)
	...
	contents, _, resp, err := client.Repositories.GetContents(ctx, user, AppConfig.GetRepoName(), "bookmarks.txt", &github.RepositoryContentGetOptions{Ref: branchRef})
	if resp != nil && resp.StatusCode == 404 {
		return ErrRepoNotFound
	}
	if err != nil {
	...
	_, _, err = client.Repositories.UpdateFile(ctx, user, AppConfig.GetRepoName(), "bookmarks.txt", &github.RepositoryContentFileOptions{
...
func (p GitHubProvider) CreateBookmarks(ctx context.Context, user string, token *oauth2.Token, branch, text string) error {
	...
	_, resp, err := client.Repositories.CreateFile(ctx, user, AppConfig.GetRepoName(), "bookmarks.txt", &github.RepositoryContentFileOptions{
		Message:   SP("Auto create from web"),
		Content:   []byte(text),
		Branch:    &branch,
		Author:    commitAuthor,
